...
export interface AppleCompletePurchaseResult {
    ...
    message?: string
    error?: string,
    appleReceiptData?: AppleVerifiedReceipt,
}
...
export function isAppleReceiptResponseRawBody(input: any): input is AppleVerifiedReceipt {
    return !isNull(input) && (input as AppleVerifiedReceipt).status !== undefined
}
...
export function getOriginalTransactionId(receipt?: AppleVerifiedReceipt): string | undefined {
    if (!receipt) {
        return undefined;
    }
...
export function isReceiptInTrial(receipt?: AppleVerifiedReceipt): boolean {
    if (!receipt) {
        return false;
    }
...
export function getCurrentSubscriptionPeriodFromReceipt(receipt?: AppleVerifiedReceipt): { startsAt?: Date, expiresAt?: Date } | undefined {
    if (!receipt) {
        return undefined;
    }
...
export enum AppleNotificationType {
     ...
     * Check expires_date to determine the next renewal date and time.
     *
     * The RENEWAL notification type is scheduled for deprecation. Update any existing code to rely on the DID_RECOVER notification type instead.
     */
    RENEWAL = "RENEWAL",
}
 ...
 *
 * NOTE: The following top-level objects and notification type are scheduled for deprecation:
 * - latest_receipt,
 * - latest_receipt_info,
 ...
 * Update any existing code to rely on the following objects in unified_receipt and notification type instead:
 * - latest_receipt,
 * - latest_receipt_info
 * - DID_RECOVER.
 *
...
export function getCancellationReasonCodeFromExpirationIntent(expirationIntent?: ExpirationIntent): CancellationReasonCode {
    if (expirationIntent === undefined) {
 ...
 */
export interface AppleVerifiedReceipt extends AppleUnifiedReceipt {
...
export interface HandleAppleNotificationResult {
    success: boolean,
}