...
import { getAppType, getAuthUser, getAuthUserId, SentryExpressHanderConfig } from "@api/util/RequestUtil";
...
app.post("/notify-friend-request", async (req: functions.https.Request | any, resp: functions.Response) => {
    ...
    // Yes, this is a little ridiculous, but I wanted to wrap the whole thing in a try/catch for
    // safety and didnt want to refactor the fucntion
    catch (error) {
        logger.error(error)
        resp.sendStatus(500)
    }
