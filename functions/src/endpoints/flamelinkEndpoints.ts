...
const signatureHandler = (req: functions.https.Request | any, resp: functions.Response, next: Function) => {
    ...
    if (!signatureHeader) {
        logger.log(chalk.yellow("FLAMELINK REQUEST HAS NO SIGNATURE - UNAUTHORIZED"));
        resp.sendStatus(401);
        return
    }
    ...
    if (!verified) {
        logger.log(chalk.red("signature invalid, returning 403"));
        resp.sendStatus(403);
        return
    }

    logger.log(chalk.green("signature valid, checking timestamp"));
...
app.post("/webhook", async (req: functions.https.Request | any, resp: functions.Response) => {
    ...
    resp.send(result);
    logger.log(chalk.green("sent response", JSON.stringify(result, null, 2)))
