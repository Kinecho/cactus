...
export default class ChangeUserEmail extends FirebaseCommand {
    ...
    async beginChange() {
        ...
        try {
            ...
            const user = await AdminUserService.getSharedInstance().getAuthUserByEmail(userInput.oldEmail);
            if (!user) {
            ...
            const existingNewEmail = await AdminUserService.getSharedInstance().getAuthUserByEmail(userInput.newEmail);
            if (existingNewEmail) {
            ...
            const updatedUser = await AdminUserService.getSharedInstance().getAuthUserByEmail(userInput.newEmail);
            if (updatedUser) {
    ...
    async changeEmail(user: UserRecord, newEmail: string) {
        // AdminUserService.getSharedInstance().em
        try {
